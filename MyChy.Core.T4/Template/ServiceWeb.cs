...
namespace MyChy.Core.T4.Template
{
    ...
    {
        private const string IPath = "/MyChy.Service.WebAdmin";
       // private const string IMappings = "/Mappings";
        ...
        private async Task CreatModuleInitializer(string Path, IList<MyChyEntityNamespace> list)
        {
            ...
            {
                sb.AppendFormat("services.AddTransient<I{0}AdminService, {0}AdminService>();", i.Namespace);
                sb.AppendLine("");
            }
        ...
        private async Task CreatIService(string Path, IList<MyChyEntityNamespace> list)
        {
            ...
            foreach (var i in list)
            {
                ...
                string files = Path + $"/I{i.Namespace}AdminService.cs";
                var _sw = new StreamWriter(new FileStream(files, FileMode.CreateNew, FileAccess.ReadWrite, FileShare.Read), Encoding.UTF8);
                ...
                sb.AppendLine("namespace MyChy.Service.WebAdmin");
                sb.AppendLine("{");
                sb.AppendLine($"public interface I{i.Namespace}AdminService : IServiceBase");
                sb.AppendLine("{");
        ...
        private async Task CreatService(string Path, IList<MyChyEntityNamespace> list)
        {
            ...
            {
                sb = new StringBuilder();
                string files = Path + $"/{i.Namespace}AdminService.cs";
                var _sw = new StreamWriter(new FileStream(files, FileMode.CreateNew, FileAccess.ReadWrite, FileShare.Read), Encoding.UTF8);
                ...
                sb.AppendLine($"public class {i.Namespace}AdminService : ServiceBase, I{i.Namespace}AdminService");
                sb.AppendLine("{");
                ...
                sb.AppendLine($"public {i.Namespace}AdminService(I{i.Namespace}Service {i.Namespace}Service");
                ...
                foreach (var x in i.FileName)
                {
                    ...
                    sb.AppendLine($"result = await _{i.Namespace}Service.Save{x.Name}PostAsync(PostModel);");
                    sb.AppendLine($"");
                    ...
                    sb.AppendLine($"");
                    sb.AppendLine($"var result = new ResultBaseModel();");
                    sb.AppendLine($"var list = await _{i.Namespace}Service.Show{x.Name}ListAsync(PostModel);");
