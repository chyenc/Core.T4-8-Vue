...
namespace MyChy.Core.T4.Template
{
    ...
    {
        private const string IPath = "/MyChy.Service.WebService";
        ...
        private async Task CreatModuleInitializer(string Path, IList<MyChyEntityNamespace> list)
        {
            ...
            {
                sb.AppendFormat("services.AddTransient<I{0}WebServiceService, {0}WebServiceService>();", i.Namespace);
                sb.AppendLine("");
            }
        ...
        private async Task CreatIService(string Path, IList<MyChyEntityNamespace> list)
        {
            ...
            foreach (var i in list)
            {
                ...
                string files = Path + $"/I{i.Namespace}WebServiceService.cs";
                var _sw = new StreamWriter(new FileStream(files, FileMode.CreateNew, FileAccess.ReadWrite, FileShare.Read), Encoding.UTF8);
                ...
                sb.AppendLine("namespace MyChy.Service.WebService");
                sb.AppendLine("{");
                sb.AppendLine($"public interface I{i.Namespace}WebServiceService : IServiceBase");
                sb.AppendLine("{");
        ...
        private async Task CreatService(string Path, IList<MyChyEntityNamespace> list)
        {
            ...
            {
                sb = new StringBuilder();
                string files = Path + $"/{i.Namespace}WebServiceService.cs";
                var _sw = new StreamWriter(new FileStream(files, FileMode.CreateNew, FileAccess.ReadWrite, FileShare.Read), Encoding.UTF8);
                ...
                sb.AppendLine("using MyChy.Service.WebService;");
                sb.AppendLine("using MyChy.Service.WebAdmin;");
                
                sb.AppendLine("");


                sb.AppendLine("namespace MyChy.Service.WebService.Implementation");
                sb.AppendLine("{");

                sb.AppendLine($"public class {i.Namespace}WebServiceService : ServiceBase, I{i.Namespace}WebServiceService");
                sb.AppendLine("{");
                ...
                sb.AppendLine($"private readonly string CacheKey = \"{i.Namespace}SWS_\";");

                sb.AppendLine($"public {i.Namespace}WebServiceService(I{i.Namespace}Service {i.Namespace}Service");
                ...
                sb.AppendLine($"_logger = loggerFactory.CreateLogger<{i.Namespace}WebServiceService>();");
