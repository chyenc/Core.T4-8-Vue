...
{
    public class ServiceWebFront
    {
        private const string IPath = "/MyChy.Service.WebFront";
        ...
        private async Task CreatModuleInitializer(string Path, IList<MyChyEntityNamespace> list)
        {
            ...
            {
                sb.AppendFormat("services.AddTransient<I{0}WebFrontService, {0}WebFrontService>();", i.Namespace);
                sb.AppendLine("");
            }
        ...
        private async Task CreatIService(string Path, IList<MyChyEntityNamespace> list)
        {
            ...
            foreach (var i in list)
            {
                ...
                string files = Path + $"/I{i.Namespace}WebFrontService.cs";
                var _sw = new StreamWriter(new FileStream(files, FileMode.CreateNew, FileAccess.ReadWrite, FileShare.Read), Encoding.UTF8);
                ...
                sb.AppendLine("namespace MyChy.Service.WebFront");
                sb.AppendLine("{");
                sb.AppendLine($"public interface I{i.Namespace}WebFrontService : IServiceBase");
                sb.AppendLine("{");
        ...
        private async Task CreatService(string Path, IList<MyChyEntityNamespace> list)
        {
            ...
            {
                sb = new StringBuilder();
                string files = Path + $"/{i.Namespace}WebFrontService.cs";
                var _sw = new StreamWriter(new FileStream(files, FileMode.CreateNew, FileAccess.ReadWrite, FileShare.Read), Encoding.UTF8);
                ...
                sb.AppendLine("using MyChy.Service.WebFront;");
                sb.AppendLine("using MyChy.Service.WebAdmin;");
                sb.AppendLine("");


                sb.AppendLine("namespace MyChy.Service.WebFront.Implementation");
                sb.AppendLine("{");

                sb.AppendLine($"public class {i.Namespace}WebFrontService : ServiceBase, I{i.Namespace}WebFrontService");
                sb.AppendLine("{");
                ...
                sb.AppendLine($"private readonly string CacheKey = \"{i.Namespace}SWF_\";");

                sb.AppendLine($"public {i.Namespace}WebFrontService(I{i.Namespace}Service {i.Namespace}Service");
                ...
                sb.AppendLine($"_logger = loggerFactory.CreateLogger<{i.Namespace}WebFrontService>();");
